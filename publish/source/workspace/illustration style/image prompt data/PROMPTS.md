...
> *"I've placed photos of my handwritten prep notes in `publish/source/workspace/`. Please parse them into a DM Session Plan for Session [XX]. Include monster HP tick-boxes, key locations, cold opens, and secrets. Save it to `publish/table/[XX]-session-name.md`."*

### Add a New Map Asset
> *"I've placed a map image in `publish/source/workspace/[map-filename.png]`. Please optimize it, place it in `publish/source/world/maps/`, add it to `publish/table-assets/table-maps.manifest`, and update any related place or module notes."*
...
> *"Here are shorthand notes from tonight's session (or see `publish/source/workspace/session-[XX]-notes.txt`). Please convert them into a clean session record at `publish/source/sessions/session-[XX]/what-happened.md`."*
